...
CMD="${SCRIPT_DIR}/../assets/nanopolish-index-0.11.1-gf.sh"
    CMD="${CMD} --reads=\"${SCRIPT_DIR}/data/test_trim.fastq.gz\""
    CMD="${CMD} --summary=\"${SCRIPT_DIR}/data/sequencing_summary.txt\""
    CMD="${CMD} --fast5=\"${SCRIPT_DIR}/data/fast5/\""
    CMD="${CMD} --output=\"${SCRIPT_DIR}/nanopolish_index\""
    CMD="${CMD} --log=\"${SCRIPT_DIR}/log\""
